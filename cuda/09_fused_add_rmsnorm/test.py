"""09_fused_add_rmsnorm CUDA 自动测试"""
...
def compile_and_run(source_file):
    ...
    result = subprocess.run(["nvcc", "-O2", "-o", exe, src], capture_output=True, text=True, cwd=DIR)
    if result.returncode != 0: pytest.fail(f"编译失败:\n{result.stderr}")
    result = subprocess.run([exe], capture_output=True, text=True, cwd=DIR)
    if os.path.exists(exe): os.remove(exe)
    return result.stdout
def test_fused_add_rmsnorm(request):
